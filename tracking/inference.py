...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            for position in self.legalPositions:
                allPossible[position]=emissionModel[util.manhattanDistance(position, pacmanPosition)]*self.beliefs[position]
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for position in self.legalPositions:
            distance = self.getPositionDistribution(self.setGhostPosition(gameState, position))
            belief = self.beliefs[position]
            for pos in distance.keys():
                allPossible[pos] += distance[pos]*belief
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:   
            for particle in self.particles:
                allPossible[particle] += emissionModel[util.manhattanDistance(particle, pacmanPosition)]
        else:        
        ...
        if self.beliefs.totalCount() != 0: 
            self.beliefs.normalize()
            numPar = 0
            while numPar < len(self.particles):
                self.particles[numPar] = util.sample(self.beliefs)
                numPar+=1
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [util.sampleFromCounter(self.getPositionDistribution(self.setGhostPosition(gameState, pos))) for pos in self.particles]
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            if len(ghosts)>0:
                particle = self.getParticleWithGhostInJail(particle, ghosts[len(ghosts)-1])
            p = 1
            i = 0
            while i < self.numGhosts:
                if i in ghosts:
                    ...
                    p *= emissionModels[i][util.manhattanDistance(particle[i], pacmanPosition)]
                i+=1
            allPossible[particle]+=p
        self.beliefs = allPossible
        if self.beliefs.totalCount() != 0: 
            self.beliefs.normalize()
            numPar = 0
